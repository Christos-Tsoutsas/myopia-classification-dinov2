# =====================================================================================
# Myopia Classification – VGG16 Baseline Experiment
# -----------------------------------------------------------------------------
# Classic CNN baseline using VGG16:
#   • ImageNet-pre-trained VGG16 backbone.
#   • Standard input resolution (224²) for VGG16.
...
REPORTS_PATH = os.path.join(PROJECT_PATH, "reports_experiment_vgg16")
MODELS_PATH = os.path.join(PROJECT_PATH, "models_experiment_vgg16")
...
IMAGE_SIZE = 224             # Standard VGG16 input resolution
HEAD_LR = 2e-4
FULL_TUNE_LR = 2e-6          # for backbone (head will get ×10)
WEIGHT_DECAY = 0.05
MODEL_NAME = "vgg16.tv_in1k"  # Torchvision's VGG16 pretrained on ImageNet
...
def main():
    ...
    print(f"[INFO] {datetime.now():%Y-%m-%d %H:%M} – Starting VGG16 Experiment on {DEVICE}")
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(trainval_df, trainval_df.label_idx)):
        ...
        print("➡️  Stage 2: fine-tuning entire VGG16")
    ...
    ax.set_title('Classification Report Heatmap', pad=40)
