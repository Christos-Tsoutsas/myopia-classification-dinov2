# =====================================================================================
# Myopia Classification – ResNet50 Baseline Experiment
# -----------------------------------------------------------------------------
# Classic CNN baseline using ResNet50:
#   • ImageNet-pre-trained ResNet50 backbone.
#   • Standard input resolution (224²) for ResNet50.
...
REPORTS_PATH = os.path.join(PROJECT_PATH, "reports_experiment_resnet50")
MODELS_PATH = os.path.join(PROJECT_PATH, "models_experiment_resnet50")
...
IMAGE_SIZE = 224             # Standard ResNet50 input resolution
HEAD_LR = 2e-4
FULL_TUNE_LR = 2e-6          # for backbone (head will get ×10)
WEIGHT_DECAY = 0.05
MODEL_NAME = "resnet50.tv_in1k"  # Torchvision's ResNet50 pretrained on ImageNet-1k
...
def main():
    ...
    print(f"[INFO] {datetime.now():%Y-%m-%d %H:%M} – Starting ResNet50 Experiment on {DEVICE}")
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(trainval_df, trainval_df.label_idx)):
        ...
        print("➡️  Stage 2: fine-tuning entire ResNet50")
    ...
    plt.title('Validation Accuracy Across All Folds - ResNet50 Experiment')
    ...
    ax.set_title('Classification Report Heatmap - ResNet50 Experiment', pad=40)
    ...
    plt.title('Normalized Confusion Matrix - ResNet50 Experiment')
    ...
    ax.text(0.95, 0.05, metrics_text, transform=ax.transAxes, fontsize=12,
            verticalalignment='bottom', horizontalalignment='right', bbox=props)
    plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate'); plt.title('ROC Curves - ResNet50 Experiment')
    plt.legend(loc="lower right")
    ...
    ax.text(0.95, 0.05, metrics_text, transform=ax.transAxes, fontsize=12,
            verticalalignment='bottom', horizontalalignment='right', bbox=props)
    plt.xlabel('Recall'); plt.ylabel('Precision'); plt.title('Precision-Recall Curves - ResNet50 Experiment'); plt.legend(loc="best")
